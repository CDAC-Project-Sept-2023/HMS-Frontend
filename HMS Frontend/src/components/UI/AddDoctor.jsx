...
function RegisterUser() {
  ...
  const DaysDropdown = ({ value, onChange }) => {
    ...
    return (
      <select value={value} name="day" onChange={onChange}>
        <option value="">Choose a day</option>
        {days.map((day, index) => (
  ...
  const OnTextChanged = (args) => {
    ...
    if (name.startsWith("schedule")) {
      const [scheduleIndex, scheduleField] = name
        .match(/\[(\d+)\]\.(\w+)/)
        .slice(1);
      copyOfDoctor.schedule[scheduleIndex][scheduleField] = value;
    } else {
  ...
  const handleAddDay = () => {
    setDoctor((prevDoctor) => ({
      ...prevDoctor,
      schedule: [...prevDoctor.schedule, { day: "", time: "" }],
    }));
  };
  ...
  const handleRemoveDay = (index) => {
    setDoctor((prevDoctor) => ({
      ...prevDoctor,
      schedule: prevDoctor.schedule.filter((_, i) => i !== index),
    }));
  };
